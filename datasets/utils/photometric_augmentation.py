# this code is mainly copied from Superpoint[https://github.com/rpautrat/SuperPoint] | removed unsupported by numpy 2.x iaa dependency
# -*-coding:utf8-*-
import cv2
from math import pi
...
def homographic_aug_pipline(img, pts, config):
    ...
    warped_valid_mask = compute_valid_mask(
        (H, W), homography, config['valid_border_margin'])
...
def compute_valid_mask(shape, homography, erosion_radius=0):
    ...
    if erosion_radius > 0:
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (erosion_radius*2,)*2)
...
def sample_homography(shape, config=None):
    ...
    if config['perspective']:
        if not config['allow_artifacts']:
            perspective_amplitude_x = min(
                config['perspective_amplitude_x'], margin)
            perspective_amplitude_y = min(
                config['perspective_amplitude_y'], margin)
        else:
            perspective_amplitude_x = config['perspective_amplitude_x']
            perspective_amplitude_y = config['perspective_amplitude_y']

        tnorm_y = stats.truncnorm(-2, 2, loc=0,
                                  scale=perspective_amplitude_y/2)
        tnorm_x = stats.truncnorm(-2, 2, loc=0,
                                  scale=perspective_amplitude_x/2)
    ...
    if config['scaling']:
        ...
        scaled = (pts2 - center)[np.newaxis, :, :] * \
            scales[:, np.newaxis, np.newaxis] + center
        if config['allow_artifacts']:
            # all scales are valid except scale=1
            valid = np.arange(config['n_scales'])
        else:
            valid = (scaled >= 0.) * (scaled < 1.)
            valid = valid.prod(axis=1).prod(axis=1)
            valid = np.where(valid)[0]
        idx = valid[np.random.randint(
            valid.shape[0], size=1)].squeeze().astype(int)
        pts2 = scaled[idx, :, :]
    ...
    if config['rotation']:
        angles = np.linspace(-config['max_angle'],
                             config['max_angle'], num=config['n_angles'])
        ...
        if config['allow_artifacts']:
            # all scales are valid except scale=1
            valid = np.arange(config['n_angles'])
        else:
...
def ratio_preserving_resize(img, target_size):
    ...
    '''
    scales = np.array(
        (target_size[0]/img.shape[0], target_size[1]/img.shape[1]))  # h_s,w_s

    new_size = np.round(np.array(img.shape)*np.max(scales)).astype(np.int32)
    ...
    # return new_img
    if curr_h > target_h:
        temp_img = temp_img[(curr_h - target_h) //
                            2: target_h + (curr_h - target_h) // 2, :]
    elif curr_h < target_h:
        pad = (target_h - curr_h) // 2
        temp_img = np.pad(temp_img, ((pad, target_h - curr_h - pad),
                          (0, 0)), mode='constant', constant_values=0)
    if curr_w > target_w:
        temp_img = temp_img[:, (curr_w - target_w) //
                            2: target_w + (curr_w - target_w) // 2]
    elif curr_w < target_w:
        pad = (target_w - curr_w) // 2
        temp_img = np.pad(temp_img, ((0, 0), (pad, target_w -
                          curr_w - pad)), mode='constant', constant_values=0)
