...
class SyntheticShapes(Dataset):
    ...
    def dump_primitive_data(self, primitive):
        ...
        for split, size in self.config['generation']['split_sizes'].items():
            im_dir, pts_dir = [Path(output_dir, i, split)
                               for i in ['images', 'points']]
    ...
    def _init_dataset(self,):
        ...
        primitives = parse_primitives(
            self.config['primitives'], self.drawing_primitives)
    ...
    def batch_collator(self, samples):
        ...
        # to tensor
        batch['raw']['img'] = torch.tensor(
            # B1HW
            batch['raw']['img'][:, np.newaxis, :, :], dtype=torch.float32, device=self.device)
        batch['raw']['kpts_map'] = torch.tensor(
            batch['raw']['kpts_map'], dtype=torch.float32, device=self.device)  # BHW
        batch['raw']['mask'] = torch.tensor(
            batch['raw']['mask'], dtype=torch.float32, device=self.device)  # BHW
        batch['homography'] = torch.tensor(
            batch['homography'], dtype=torch.float32, device=self.device)  # B33
...
if __name__ == "__main__":
    ...
    for i, d in enumerate(data_loaders['train']):
        if i >= 100:
            break
        img = (d['raw']['img'][0] *
               255).cpu().numpy().squeeze().astype(int).astype(np.uint8)
        ...
        mask = d['raw']['mask'][0].cpu().numpy(
        ).squeeze().astype(int).astype(np.uint8)*255
